...
and parseEX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    | EqualToken ->
        ...
        parseEX
    (* regle #7 *)
    | (RightParenthesisToken | ThenToken | ElseToken) -> inject stream
...
and parseARG stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    (* regle 26 *)
    (* ARG -> *)
    | (RightParenthesisToken | EqualToken | PlusToken | MinusToken | TimesToken | DivideToken | ElseToken | ThenToken | InToken) ->
      inject stream
    | EOSToken -> inject stream
