...
let acceptIdent stream =
  match (peekAtFirstToken stream) with
    | (IdentToken name) -> (print_endline ("accept " ^ name));(Success (advanceInStream stream))
    | _ -> Failure
;;
...
let acceptNumber stream =
  match (peekAtFirstToken stream) with
    | (NumberToken value) -> (print_endline ("accept " ^ (string_of_int value)));(Success (advanceInStream stream))
    | _ -> Failure
;;
  ...
  *)
  ...
  (match (peekAtFirstToken stream) with
    ...
    (* Symboles directeurs *)
       ...
       | FunctionToken -> 
        (print_endline "function ident fleche E");
        (match (accept FunctionToken stream) with
              | (Success next) -> (print_endline "ident fleche E");(match (acceptIdent next) with
                  | (Success next) -> (print_endline "fleche E");(match (accept BodyToken next) with
    ...
    | IfToken -> 
      (print_endline "if E then E else E");
      (match (accept IfToken stream) with
            | (Success next) -> (print_endline "E then E else E");(match (parseE next) with
                | (Success next) -> (print_endline "then E else E");(match (accept ThenToken next) with
                   | (Success next) -> (print_endline "E else E");(match (parseE next) with
                      | (Success next) -> (print_endline "else E");(match (accept ElseToken next) with
                        | (Success next) -> (parseE next)
                        | _ -> Failure)
                      | _ -> Failure)
                    | _ -> Failure)
...
and parseEX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    (match (accept EqualToken stream) with
       ...
       | _ -> Failure)
    (* regle #7  - EX ->          | { $, ) } *)
    | EOSToken | RightParenthesisToken | ThenToken | ElseToken -> (print_endline "");(Success stream)
    | _ -> Failure)
...
and parseTX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    | PlusToken  -> (print_endline "T TX");  (match (accept PlusToken stream) with
            | Success next -> (print_endline "TX"); (match (parseT next) with
                | Success next -> (parseTX next)
                | _ -> Failure)
            | _ -> Failure)

    | MinusToken -> (print_endline "T TX");  (match (accept MinusToken  stream) with
             Success next -> (print_endline "TX"); (match (parseT next) with
                | Success next -> (parseTX next)
                | _ -> Failure)
            | _ -> Failure)
...
and parseFX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    | TimesToken  -> (print_endline "F FX");  (match (accept TimesToken stream) with
            | Success next -> (print_endline "FX"); (match (parseF next) with
                | Success next -> (parseFX next)
                | _ -> Failure)
            | _ -> Failure)
    | DivideToken -> (print_endline "F FX");  (match (accept DivideToken  stream) with
            | Success next -> (print_endline "FX"); (match (parseF next) with
                | Success next -> (parseFX next)
                | _ -> Failure)
            | _ -> Failure)
...
and parseF stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    | IdentToken _ -> (print_endline "ident");(acceptIdent stream)
    (* regle #21 - F -> true      | { true } *)
    | TrueToken -> (print_endline "true");(accept TrueToken stream)
